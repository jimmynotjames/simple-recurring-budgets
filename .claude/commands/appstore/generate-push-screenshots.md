...
- Upload axis: `upload-now` → capture, then upload immediately (no inspection
  ...
  "proceed" before uploading. No upload argument → the skill asks the
  pause-vs-upload choice once. (If your message already says to upload without
  stopping, the skill treats that as `upload-now`.)
- Capture axis (only matters when shots already exist): `recapture` → clear the
  ...
  skip capture. No capture argument → if capture is already done, the skill asks
  re-capture vs reuse once. (If your message already says to re-capture/overwrite or
  to keep, the skill treats that as the matching argument.)
...
3. Capture via `fastlane screenshots` (~2 h). If capture is already done, the skill
   asks whether to re-capture (overwrite) or keep the existing shots — capture is
   skipped only when you keep them. Self-heals any flaked locales, tracked on
   20-minute ticks
