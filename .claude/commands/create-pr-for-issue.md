...
5. Push the branch and open the PR. **Write the body to `tmp/pr-body.md` with the Write tool, then `gh pr create --title "…" --body-file tmp/pr-body.md`** — never pass the body inline via `--body`, because PR bodies contain backticks (command-substitution syntax) and newlines that force a permission prompt even though `gh pr *` is allowlisted. Use the PR-description template from `AGENTS.md`, with issue linkage in the body: `Closes #$1` for a complete fix, or `Refs #$1` plus a "what's still pending" note for a partial one.
6. **Stop. Do not merge.** Report the PR URL and a one-line summary, then hand back for manual review. The `/merge-pr` command lands it after you've reviewed.
