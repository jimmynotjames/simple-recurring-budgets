...
id: appstore-screenshot-content
category: App Store
...
Follow the **`appstore-screenshot-content`** skill recipe end to end
(`.cursor/skills/appstore-screenshot-content/SKILL.md`). If locale arguments were
passed (e.g. `ja de-DE ar-SA`), generate only those storefronts; otherwise
generate every missing locale.

On Cursor (no subagent primitive), do step 3 **inline and serially**: for each
