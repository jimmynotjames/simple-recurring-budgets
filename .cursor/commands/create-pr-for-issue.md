...
Take GitHub issue `#<N>` (the issue number you pass to this command) from report to an open, review-ready PR.
...
8. Push the branch and open the PR. **Write the body to a file `tmp/pr-body.md`, then `gh pr create --title "…" --body-file tmp/pr-body.md`** — never pass the body inline via `--body`, because PR bodies contain backticks (command-substitution syntax) and newlines that force a permission prompt even though `gh pr *` is allowlisted. Use the PR-description structure from `.github/pull_request_template.md`, with issue linkage: `Closes #<N>` for a complete fix, or `Refs #<N>` plus a "what's still pending" note for a partial one.
