...
6. **Gate** — `make gate` passes clean *(or skipped after docs-only CI confirmation)*
...
- **If yes:** ensure the HEAD commit includes `[skip ci]` (amend or a small follow-up commit before push). You may skip the local gate (step 6) — no app code to validate.
