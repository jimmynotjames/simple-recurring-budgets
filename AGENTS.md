...
11. **Prompt about lingering OpenSpec steps** — if the merged work implemented an OpenSpec change, that change stays **active until archived**. Run `openspec list`; if a change tied to this PR is still active (a directory under `openspec/changes/<name>/`, not yet under `archive/`), **stop and ask the user** whether to finish it now — `/opsx:archive <name>` syncs the delta specs into the main specs and moves the change to `openspec/changes/archive/` — or defer. This is a deliberate exception to the "don't prompt during workflows" default: always surface the pending archive rather than auto-archiving silently or skipping it. (Archiving lands spec edits on `main`, so it goes through its own branch + PR per § Never push directly to `main`.)
