...
- **App Store metadata** — `appstore-translate-metadata` skill (`/appstore:translate-metadata`) →
  `scripts/translate_metadata/` (structural `audit.py` + semantic `audit_semantic.py`). Gate:
  `check_metadata.py`.
- **App Store screenshot demo-content** — `appstore-screenshot-content` skill
  (`/appstore:screenshot-content`) → `scripts/screenshot_content/`. Gate: `check_content.py`.
