...
| `expense_recent_reused`     | User tapped a Recents suggestion tile in Add Expense (F-7.04). Fires on tap regardless of whether the user later Saves; pairs with the subsequent `expense_logged` if Save happens. Answers the "do users actually reuse recents?" question. | F-7.04, §3.1                             |
...
| `expense_recent_reused` | `recents_visible_count`      | `0` / `1` / `2-3` / `4-7` / `8-15`           | Bucketed count of tiles in the Recents row at tap time. Answers "do users tap into a dense or sparse row?" Bucket boundaries are coarse on purpose to avoid pseudo-identifying signatures. |
