...
| `expense_recent_reused`     | User tapped a Recents suggestion tile in Add Expense (F-7.04). Fires on tap regardless of whether the user later Saves; pairs with the subsequent `expense_logged` if Save happens. Fires whether the tap filled the amount or preserved a user-typed one (provenance rule). Exactly one event per physical interaction: the double-tap full replace does **not** emit a second event (its first tap already did). Answers the "do users actually reuse recents?" question. | F-7.04, §3.1                             |
