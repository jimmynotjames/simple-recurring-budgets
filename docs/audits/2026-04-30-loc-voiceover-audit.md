...
- **L-05 (MEDIUM)** — Replace `Text("\(appVersion) (\(buildNumber))")` with an explicit `Text(String(localized: "settings.version.value", defaultValue: "\(appVersion) (\(buildNumber))", comment: "Renders the app version and build number on the Settings About row; arguments are the version string and the build number"))` so the catalog key is project-namespaced and translator-friendly.
