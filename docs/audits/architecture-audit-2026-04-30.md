...
2. **All persistence saves silently discard errors.** Every `context.save()` call in the app uses `try?`. There is no user-facing error state, no retry, and no logging of save failures. If you add a new write path, be aware of this pattern and its risks. Do not assume saves always succeed.
