...
---
## Tasks for Agent
Using the `/opsx:explore` skill, create a spec to write the algorithm in this doc. 
...
| **Effective start date** (`effectiveStartDate`) | `budget.startDate ?? budget.createdAt` — the algorithm's resolved start (§A.4.1 step 1). The `?? createdAt` is a safety net for malformed sync records; the UI guarantees `startDate` is set. |
| **Effective now** (`effectiveNow`) | `min(now, budget.endDate ?? .distantFuture)` — clock time clamped at `endDate` so the chip freezes after a budget ends (§A.4.1 step 4). |
...
3. Backdated expenses — including into prior active periods of a currently-paused budget —
   propagate correctly through the carry-over (§2.7, §6.7.5).
...
@Model final class AllocationChange {
  var id: UUID = UUID()
  /// The period-boundary date at which this allocation becomes effective.
  /// For the initial entry, equal to `effectiveStartDate` (start-of-day normalized).
  /// For subsequent entries, equal to the `currentPeriodStart` at the time the edit was
  /// committed (period boundaries are already start-of-day for daily/weekly/biweekly and
  /// start-of-month for monthly).
...
- A saved `Budget` always has **at least one** `AllocationChange`. The first one — created in the
  same transaction as the budget itself — has `effectiveFrom == effectiveStartDate` (start-of-day
  normalized; see §A.4.0) and `amount == initialAllocation`.
- Adding a new entry uses the same period-boundary key (`effectiveFrom`) as the period in progress
  ...
  "latest-wins within the same period" rule (§6.2.3).
...
@Model final class LifecycleEvent {
  var id: UUID = UUID()
  /// Stored as `LifecycleEventKind.rawValue` ("pause" | "resume").
  var kind: String = LifecycleEventKind.pause.rawValue
  /// The user-action timestamp. See "pre-start pause" handling below.
  var date: Date = Date()
  /// User-action timestamp for the *row*, set on insert. **Tiebreak** when two events share the
  /// same `date` — e.g. a pause on one device and a resume on another at the same instant, or
  /// two pre-start pauses across devices (both stored at `startDate − ε`). Latest `lastModified`
  /// wins for post-period classification.
  var lastModified: Date = Date()
  @Relationship(deleteRule: .nullify) var budget: Budget?
...
- Sort at read time by `(date, lastModified)` ascending. CloudKit-late arrivals re-sort naturally
  on next read.
- **Pre-start pause carve-out.** If the user invokes Pause while `now < startDate`, the event is
  stored with `date == startDate - 1.nanosecond` (or equivalent ε before `startDate`). This places
  it logically *before* the first period rather than *in* it, so the algorithm's "any event in
  this period → period is active" rule does not apply and the first period correctly starts
  paused (§5.5 "Pause while `startDate` is in the future"). The user-facing label can render the
  pause as "Paused on `startDate`" — only the stored timestamp is offset.
- **Concurrent pause/resume across devices — deterministic tiebreak.** Both events are persisted
  and sorted by `(date, lastModified)` ascending on read. Two events sharing the same `date` are
  ordered by `lastModified` (later wins). For per-period classification (Rule 1 / Rule 2 of
  §A.5.4) the algorithm processes them in this stable order. No "phantom paused period" is
  possible because the classification only depends on which events fall inside the period and
  which is the latest event before it — and the sort is now deterministic.
...
- `LifecycleState.postEnd` takes precedence over `.paused` when both would apply (briefing
  §6.7.12: "if `endDate` falls inside a paused period, no special handling is required — the
...
4. **Clamp `now` to the last active day.** `effectiveNow = min(now, effectiveEndInclusive ?? .distantFuture)`.
   ...
   identifies the *final active day's* period as "current" — `periodStart(containing:
   effectiveNow)` lands inside the final period rather than past it. (Note: while clamping is
...
7. **Classify the current period** (§A.5.4). `isCurrentPaused = !isActive(periodStart:
   effectivePeriodStart, periodEnd: currentPeriodEnd, events: lifecycleEvents)`. Note: pass
   `effectivePeriodStart` (i.e. `max(currentPeriodStart, effectiveStartDate)`), **not**
   `currentPeriodStart`. This ensures a pre-start pause event stored at `startDate − ε` is
   correctly classified as "before the first period" even when `startDate` falls mid-period
   (e.g. a monthly budget with `startDate = April 15` — see §A.5.4 "Pre-start pause").

8. **Look up the current allocation** (§A.5.2). `effectiveAllocation = allocationInEffect(at:
   max(currentPeriodStart, effectiveStartDate), history: allocationChanges)`. The clamp to
   `effectiveStartDate` matters only when `startDate` falls mid-period (e.g. monthly with a
   mid-month start: a $100/mo budget with `startDate = June 16` should show its full $100
   allocation **on** June 16, not after rebound to July 1). Without the clamp,
   `currentPeriodStart` is `June 1` — strictly before the initial `AllocationChange.effectiveFrom
   = June 16` — and the lookup misses the initial entry, relying on the defensive
   `sorted.first?.amount` fallback for what is actually a normal flow. With the clamp, the first
   period lands directly on the initial entry. The clamp is a no-op for every subsequent period.
...
9. **Compute `remaining` for the current period.**
   ...
   - Else: `currentPeriodExpenses = sum of e.amount for e in expenses where
     effectivePeriodStart <= e.date < effectivePeriodEnd`. `remaining = effectiveAllocation −
     currentPeriodExpenses`. (Negative results are valid — overspending.)
...
2. `effectiveAllocation = allocationChanges.sorted { ($0.effectiveFrom, $0.lastModified) < ($1.effectiveFrom, $1.lastModified) }.last?.amount ?? 0`
   (latest-wins by `effectiveFrom`, with `lastModified` as the tiebreak; §5.3 / §6.6.7,
   §A.2.2). The walker is bypassed entirely; allocation history is consulted only for its most
   recent entry. In normal flow there is exactly one entry (per §A.6.2 step 1 — Specific Dates
   allocation edits mutate the single row), so the `lastModified` tiebreak is defensive against
   the rare cross-device duplicate where two devices each inserted their own row before sync.
...
5. `carryOver = nil`. The signed cumulative chip is not surfaced for this type (§A.5.1, §6.7).
   The asymmetric spillover rule (§A.5.6) does not apply — Specific Dates has only one period,
   so the concept of "Carry-over from prior periods" is meaningless.
...
7. `lifecycleState = (now >= effectiveEndExclusive) ? .postEnd : .active`. Note `>=` (not `>`):
   matches §A.4.1 step 11 priority 1. Pause/Resume cannot apply (§2.5 / §6.6.8) — if a stray
   `LifecycleEvent` exists from a CloudKit write or UI bug, it is ignored.
...
that the latest edit within the same period wins (§6.2.3). Across devices, two rows at the same
...
static func walkCarryOver(
  ...
  for (i, boundary) in boundaries.enumerated() {
    ...
    // Full per-period allocation — no proration for partial first/last periods (§2.4).
    // The clamp `max(boundary, effectiveStartDate)` ensures a partial first period (e.g. a
    // monthly budget with `startDate = June 16` whose natural period boundary is `June 1`) still
    // receives the full per-period allocation — $100, not a prorated $50. The lookup at that
    // date lands directly on the initial `AllocationChange` entry whose
    // `effectiveFrom == effectiveStartDate`. For every subsequent period
    // `boundary >= effectiveStartDate`, so the clamp is a no-op and the lookup uses the period's
    // natural start.
...
- **Backdated expenses (§2.7, §6.7.5–6).** The walker filters expenses fresh on every call;
  changing an expense's date or amount immediately changes the sum for whichever active period
  it now falls in.
- **Reset Carry-Over floor (§6.3.4).** `walkStart = max(effectiveStartDate, lastResetDate ?? .distantPast)`
...
- **Paused periods (§2.7, §5.5, §6.7.7).** The walker skips paused periods entirely. Their
  contribution is 0 — including any expenses that somehow ended up dated within them.
- **No proration (§2.4, §6.5.3).** Partial first/last periods get the full allocation.
...
static func isActive(
  ...
  // Primary sort: date ascending. Secondary (tiebreak): lastModified ascending so the later
  // writer wins when two events share a `date` (cross-device race; see §A.2.3).
...
- **Multiple events in one period.** Rule 1 returns true (the period is active). The
  ...
  period.
- **Pre-start pause.** The pause is stored with `date == startDate − ε` (§A.2.3). Callers pass
  the *effective* lower bound `max(naturalPeriodStart, startDate) = startDate` for the first
  period (per the caller contract above). The event at `startDate − ε` is **not** in
  `[startDate, periodEnd)` — Rule 1 returns false — and Rule 2 finds it as the latest prior
  event, returning false (paused). The first period is therefore paused, consistent with "from
  `startDate` onward the budget is in a paused state until the user resumes" (§5.5). Without the
  clamp, a monthly budget with a mid-month `startDate` (e.g. `April 15`) would land the
  pre-start event inside the first natural period (`April 15 − ε` ∈ `[April 1, May 1)`) and
  Rule 1 would incorrectly return active. The clamp is the fix.

**Concurrent device convergence (§6.2.12 / §6.7.13).** Because `isActive` re-sorts the events on
every call, it is insensitive to the order in which CloudKit delivers them. Two events with the
same `date` are tie-broken **deterministically** by `lastModified` (later writer wins). If two
same-`date` events with different `kind` (e.g. a Pause on one device and a Resume on another)
arrive after a cross-device race, the one with the later `lastModified` becomes the latest event
for Rule 2's "post-period state" lookup, so every device converges to the same paused/active
classification.
...
"Frozen" in `.postEnd` is **intrinsic to the algorithm** (because `effectiveNow = min(now,
effectiveEndInclusive)` stops advancing) rather than a UI-only effect — but backdated edits
within `[effectiveStartDate, effectiveEndExclusive)` still recompute (§2.9, §6.5.2). The same
...
Additionally, when a write inserts or mutates an `AllocationChange` or `LifecycleEvent` row, that
row's own `lastModified` is set to `now` in the same `save()`. This is what makes the
cross-device tiebreak in §A.5.2 / §A.5.4 deterministic — the later writer's row carries the
later `lastModified` and wins the sort. Per-section steps below mention this only where the
distinction matters (e.g. mutating an existing `AllocationChange` vs. inserting a new one).
...
2. Insert one `AllocationChange(effectiveFrom: effectiveStartDate, amount: initialAllocation)`
   linked to the budget, where `effectiveStartDate = calendar.startOfDay(for: startDate)` — i.e.
   the same normalized form the algorithm uses on read (§A.4.0). Its `lastModified` defaults to
   `Date()` on insert.
3. `save()`.
...
1. If `period == .specificDates`: **mutate** the single `AllocationChange` — set both `amount`
   and `lastModified = now` (latest-wins; §6.6.7). `save()`. No other steps.
2. Else (recurring):
   - **Pre-start (`now < effectiveStartDate`).** Mutate the initial `AllocationChange` at
     `effectiveFrom == effectiveStartDate`: set both `amount` and `lastModified = now`. Do **not**
     compute `currentPeriodStart` — it is meaningless before the budget has started, and the
     initial entry is the only one that exists for a never-started budget. `save()`.
...
Post-reset rebound (§6.3.1) is automatic: the current period's `remaining = allocation −
currentPeriodSpend` is independent of the walker and still reflects today's existing expenses.
...
1. Resolve `effectiveDate`:
   ...
   - If `now < effectiveStartDate`: `effectiveDate = effectiveStartDate - ε` (§A.2.3 pre-start
     carve-out).
2. Insert `LifecycleEvent(kind: .pause, date: effectiveDate)`. Its `lastModified` defaults to
   `Date()` on insert — the wall-clock instant of the user action, which is the tiebreak field
   for cross-device same-`date` collisions (§A.5.4).
3. `save()`.
...
1. Insert `LifecycleEvent(kind: .resume, date: now)`. Its `lastModified` defaults to `Date()` on
   insert (same as Pause).
2. `save()`.

Rejected if `now >= effectiveEndExclusive` (§5.5, §6.7.12) — UI-enforced. Algorithm ignores any
stray such event on `.specificDates`.
...
| 3 | Multiple edits in same period                                         | Replace existing entry at `currentPeriodStart` (§A.5.2). Latest wins; prior figure not retrievable (§6.2.3).  |
...
| 4 | Edit pre-reset expense in a *prior completed* period → carry-over unaffected | `walkStart = max(effectiveStartDate, lastResetDate)` excludes any prior completed periods that ended before `lastResetDate` (§A.5.3). The period in which the reset itself occurred is still walked once it completes — briefing §6.3.4 requires earlier periods to be unaffected, not the reset period. Briefing §6.3.1 documents the in-reset-period rebound. |
...
| # | Case                                                       | Handled by                                                                                                    |
| - | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| **1 ★** | `now > endDate`                                       | `effectiveNow = min(now, effectiveEndExclusive)` (step 4). Walker stops at the period containing `endDate`. Chip frozen. `.postEnd` fires once `now >= effectiveEndExclusive` (§A.4.0 + step 11). |
...
| 13 | Concurrent pause on one device, resume on another               | Both events sync via CloudKit; sorted by `date` on read. `isActive` is sort-stable (§A.5.4). No phantom paused period. |
...
- `isActive(periodStart:periodEnd:events:)`:
  ...
  - Multiple events in one period → "post-period state" picks last by `(date, lastModified)`.
  - Sort-order independence (input shuffled, result identical).
  - **`lastModified` tiebreak**: two events with the same `date` but different `kind` — later
    `lastModified` wins for Rule 2's "latest prior event" classification.
...
- Verify Reset Budget: expenses deleted; allocation history and lifecycle events untouched;
  `lastResetDate` set.
- Verify `lastModified` bump on every write: mutating an `AllocationChange.amount` bumps its
  `lastModified`; inserting a `LifecycleEvent` sets its `lastModified` to `now`; both also bump
  `budget.lastModified` in the same `save()`.
...
- **Two `AllocationChange` rows with the same `effectiveFrom`.** Construct two rows with distinct
  ...
  with the input array shuffled — result must be identical.
- **Two `LifecycleEvent` rows with the same `date` but different `kind`.** Construct one
  `.pause` and one `.resume` at the same `date` with distinct `lastModified` values; assert that
  `isActive` for the *next* period reflects the kind of the row with the **later `lastModified`**
  (Rule 2's "latest prior event"). Shuffle input — result identical.
- **A `LifecycleEvent` row whose `date` is between two other events.** Standard sort-order
  invariance test for `isActive`.

The contract these tests pin down: **convergence to a specific value**, not just "some value."
Both tiebreaks are `lastModified ascending → later wins` (§A.5.2 / §A.5.4).
...
3. **Should `effectiveAllocation` for `.specificDates` be the latest entry, or the
   ...
   only if the user edits the allocation, and §6.6.7 explicitly mandates latest-wins.
