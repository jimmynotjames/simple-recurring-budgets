...
6. **F-5.01 weekStartDay scope.** F-5.01: the global setting "will cascade to all existing Budgets." Under the briefing, weekly/biweekly Budgets derive their anchor from `startDate`, ignoring the global setting at math-time. `AppSettings.weekStartDay` still seeds the UI's pre-populated `startDate` at creation time (per §2.4). It is no longer consulted as a math-time fallback; if `startDate` is somehow nil at read time, the algorithm falls back to `createdAt`. F-5.01 Edge Cases / Notes should be updated to describe the per-budget override and the pre-population behavior. **[Superseded 2026-06 by issue #240 / change `weekly-global-week-start`: this decision was reversed for weekly budgets — the global setting now defines the weekly grid at math-time; biweekly remains `startDate`-anchored. See F-5.01 and the algorithm doc §A.4.1 step 5.]**
