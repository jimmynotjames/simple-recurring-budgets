...
Keeping the spec in sync is equally required: any change that alters **what** analytics emits — a new or changed event, a property added to or removed from an existing event, **a new value in a categorical property's enum** (e.g. a new `BudgetPeriod` adding a `period` value, or a new `icloud_state`), changed bucket boundaries, or a new super/people property — must update [`docs/analytics-spec.md`](analytics-spec.md) (§9–§10) **in the same change**, with a Revision-History entry. This catches drift that the "fire `track(...)` for new actions" rule misses, because adding an enum value to an already-instrumented property ships no new action.
