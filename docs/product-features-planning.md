...
- **Acceptance Criteria:**
  ...
  - **Inactive presentation** — all three lifecycle states where the budget is not currently running (`.preStart`, `.paused`, `.postEnd`) share a unified row treatment driven by a view-layer `BudgetInactiveReason` (see change `unify-inactive-budget-states`):
    - The large amount renders in `.foregroundStyle(.secondary)` (dimmed) regardless of sign. For `.preStart` / `.postEnd` it shows the budget's allocation; for `.paused` it shows `remaining` (the calculator's in-period figure).
    - The `RemainingBar` renders as a full-width `.secondary`-filled capsule rather than a fraction-driven bar.
    - The status chip area shows a single `InactiveStatusChip` carrying the reason: "Starts {date}" (`calendar.badge.clock`), "Paused · {date}" (`pause.circle.fill`), or "Ended {date}" (`checkmark.circle`).
    - The `CarryOverChip` (when applicable per F-2.01's carry-over rules) continues to render with its value dimmed to `.secondary` and its surplus / deficit capsule tint preserved. Backdated edits to prior active periods may still change the carry-over value while the row is inactive.
    - Specific Dates budgets viewed before `startDate` or after `endDate` receive the same `.preStart` / `.postEnd` treatment as recurring budgets (one chip, one rule). The `CarryOverChip` remains hidden for Specific Dates per F-2.08.
    - The inactive presentation does not affect the row's drill-in tap target, the per-row Add Expense button, or the swipe / reorder affordances.
...
- **Acceptance Criteria:**
  ...
  - **Inactive presentation** (status header) — all three lifecycle states where the budget is not currently running (`.preStart`, `.paused`, `.postEnd`) share a unified status-header treatment driven by a view-layer `BudgetInactiveReason` (see change `unify-inactive-budget-states`). Mirrors the F-2.01 row treatment:
    - The large amount renders in `.foregroundStyle(.secondary)`. For `.preStart` / `.postEnd` it shows the budget's allocation; for `.paused` it shows `remaining`.
    - The `RemainingBar` renders as a full-width `.secondary`-filled capsule.
    - The header's status chip area shows a single `InactiveStatusChip` carrying the reason: "Starts {date}", "Paused · {date}", or "Ended {date}".
    - The inline `CarryOverChip` (when applicable) continues to render with its value dimmed to `.secondary` and its surplus / deficit capsule tint preserved; value remains live via backdated edits to prior active periods.
    - The inactive presentation governs presentation **only**. The primary-action slot's existing state-driven behavior (Active / Pre-start / Post-end → "Add Expense"; Paused → "Resume Budget" + caption) is unchanged — preStart / postEnd do not introduce a new primary-slot variant.
  - **Status header** — large monospaced remaining amount (deficit-tinted when negative), period label, fuel-gauge `RemainingBar`, and — when carry-over is enabled and the period type is recurring — an inline `CarryOverChip`. The manual carry-over reset is triggered from the toolbar overflow Menu (see "Reset Carry-Over…" below), not from a header control.
  ...
  - **Toolbar overflow Menu** (`ellipsis.circle`, top-trailing) hosting:
    ...
    - **Reset Carry-Over…** (destructive) — clears this budget's carry-over balance to zero by setting `lastResetDate = now`. No `ExpenseItem`s are deleted. Hidden when `Budget.isCarryOverEnabled == false`; otherwise visible regardless of the live carry-over magnitude or sign. Available while paused. See [main-prd.md §6.7](main-prd.md#67-carry-over-behavior).
  ...
  - **Reset Carry-over** lives in the toolbar overflow Menu as "Reset Carry-Over…" (see above), gated by `Budget.isCarryOverEnabled == true`. Activating it presents a single-button destructive confirmation alert; on confirm, only this budget's carry-over is cleared (per [main-prd.md §6.7](main-prd.md#67-carry-over-behavior)). Hidden for Specific Dates budgets (which do not have carry-over enabled). Available while paused.
...
- **Acceptance Criteria:**
  ...
  - Fields:
    ...
    - **Period type** (`daily`, `weekly`, `biweekly`, `monthly`, `specificDates`). Defaults to `daily`. **Period type is immutable after creation** for every option (including `specificDates`). In Edit mode the period chips are non-interactive (rendered as static labels, not buttons) with a muted visual style; a `lock.fill` caption ("Period type can't be changed after creating your budget.") is displayed below the chip grid. The caption names the *period type* specifically because the Start/End dates remain editable in Edit mode for every period type — only the period type is immutable. The model layer also excludes `period` from the fields written on Edit-mode Save — period cannot be mutated regardless of any UI or programmatic state. (For `specificDates` semantics see F-2.08.)
    - **Start Date** — required for every period type. Editable in both Add and Edit modes. Pre-population in Add mode varies by period type:
      - **Daily** — today.
      - **Weekly / biweekly** — the latest natural anchor implied by `AppSettings.weekStartDay` (i.e., the most recent past or current `weekStartDay`-aligned date). For biweekly budgets the saved `startDate` is the cycle anchor (`biweeklyAnchor` = `startDate`). Weekly budgets grid on the global `AppSettings.weekStartDay` at math-time (change `weekly-global-week-start`, F-5.01); a weekly `startDate` off that grid clips the budget's first period without proration. In **Add mode**, if the user changes the global week-start mid-create via the in-form Start of Week picker (F-5.01, change `weekly-week-start-from-budget-form`), the **weekly** draft's `startDate` re-anchors to the most-recent-past occurrence of the new week-start (`AddEditBudgetViewModel.realignWeeklyStartDate(to:)`). This re-anchor is Add-mode and weekly only — it is a no-op in Edit mode (the existing `startDate` is user data) and for non-weekly periods.
      ...
      - **Orphan-expense warning (recurring period types, Edit mode):** when the user moves Start Date forward past one or more existing expenses, the affected expenses become **orphaned** — they remain visible in the Budget Detail expense list but are excluded from carry-over and remaining (the calculator filters by `effectiveStartDate`). To surface the consequence: (a) an inline `.caption` warning renders inside the expanded Schedule disclosure while the draft is in the orphaning state, and (b) tapping Save presents a confirmation alert (`"Start date is after N logged expenses"` / `"Those expenses still show in your list but won't be counted by this budget."` / `Cancel` · `Save Changes`); confirming proceeds with the save unchanged, cancelling preserves the draft. On re-open, if a budget is already in an orphaned state, the Schedule disclosure auto-expands. Shipped by change `add-orphan-expense-warning`. The symmetric End Date case (moving End Date earlier past expenses) is a known parallel gap, deferred.
      - **Biweekly start-date UX (Edit + Add):** because biweekly is the only recurring period whose 14-day grid is anchored to the budget's own `startDate` (see F-7.05; daily resets daily, weekly grids on the global Week-Starts-On setting per F-5.01, monthly on the calendar month), the start date is surfaced and its consequence made explicit:
        - **Explanatory note** — when Biweekly is selected, a `.caption` note ("Repeating 14-day period. Choose your start date below to choose which day each cycle begins on.") renders under the period chips in both Add and Edit modes.
        - **Schedule auto-expand** — selecting Biweekly (or opening Edit on a biweekly budget) auto-expands the Schedule disclosure so the start date (the cycle anchor) is visible; auto-expand only, never auto-collapse.
        - **Save-time re-anchor confirmation** — in Edit mode, when a biweekly budget's Start Date is changed, tapping Save presents a confirmation ("Change Start Date?" / "This will re-align current and future two-week cycles and recalculate past periods." / `Cancel` · `Change`) before committing, since the edit re-anchors the grid and recomputes past periods (and carry-over). When the same edit also strands expenses, the orphan sentence folds into this one alert (it supersedes the standalone orphan warning for biweekly). Shipped by change `biweekly-startdate-ux`.
...
- **Acceptance Criteria:**
  ...
  - **Calendar section** — Week Starts On picker: shows a confirmation alert before applying the selected weekday (the alert states the cascade: the change also affects existing weekly budgets and recalculates their past weeks); confirmed value writes to `AppSettings.weekStartDay`; see F-5.01 for week-start semantics.
  ...
  - **Support section** — "Send Feedback" (opens `mailto:` link), "Rate the App" (invokes `requestReview()`), "Privacy Policy" (opens the privacy policy URL in browser).
...
- **Status:** Implemented. `AppSettings.weekStartDay` with Settings picker and confirmation alert; implemented by change `settings-screen`. Global-grid scope (below) shipped with `weekly-global-week-start` (issue #240), reversing the per-budget-anchor scope clarification that had shipped with `rewrite-budget-calculations`. A second entry point — the Add/Edit Budget weekly disclosure note + in-form Start of Week picker — shipped with `weekly-week-start-from-budget-form`.
- **Description:** Choose the start day of week. The setting defines the **global weekly grid**: every weekly Budget's periods roll on this weekday, the same way every monthly Budget shares the calendar-month grid.
- **Acceptance Criteria:**
  ...
  - **Scope:** the setting is consulted at math-time for **all weekly budgets** — `BudgetCalculator.snapshot` and the `BudgetLifecycleService` entry points take it as an explicit `weekStart` parameter (no default), and view call sites pass `settings.weekStartDay`. Changing it **immediately re-grids every weekly budget, past periods included**; historical per-week sums regroup and carry-over recomputes (the confirmation alert states this). A weekly `startDate` off the grid merely clips the budget's first period (full allocation, no proration). **Biweekly budgets are unaffected**: their 14-day cycle stays anchored to their own `startDate` — a weekday cannot define the cycle's phase. The setting also still seeds the pre-populated `Start Date` for new weekly/biweekly budgets (see F-2.03).
  - This setting is stored in `NSUbiquitousKeyValueStore` and syncs automatically across the user's iCloud-connected devices. Budgets-list rows and Budget detail refresh on `onChange(of: settings.weekStartDay)`, so both local and remote-synced changes re-grid visible budgets immediately.
  - **Entry points:** changeable from two surfaces — (1) Settings → Calendar "Week Starts On" menu picker, and (2) the Add/Edit Budget screen, where selecting the Weekly period reveals a disclosure note ("All weekly budgets start on `<day>`.") with a "Change start of week" link that pushes a focused Start of Week screen (`WeekStartPickerScreen`) onto the form's own navigation stack (the in-progress draft is preserved). Both surfaces write the same global `AppSettings.weekStartDay` behind the same cascade-warning confirmation (`WeekStartConfirmation` + the shared `settings.weekStart.alert.*` strings) and fire the same `setting_changed` (`week_start_day`) analytics; the focused screen carries an app-wide-scope banner so its global reach is unambiguous. The two surfaces intentionally use different layouts (menu picker vs. full-screen list).
- **Edge Cases / Notes:**
- **Dependencies:**
...
- **Status:** Implemented. Implemented by change `finish-recently-used-expenses` (workshop design landed pre-change; cross-cutting concerns and tests landed with the change). Reworked by change `recents-rework-spec-sync` (placement below Description, recurrence-gated amount variants, corpus/display cap split, amount-provenance smart apply, double-tap full replace, Description clear button). Extended to Edit mode by change `recents-in-edit-mode` (blank-Description-gated reveal in Edit/View mode).
- **Description:** Surface the user's recently logged expenses as tappable suggestions in Add Expense (always) and Edit/View Expense (when the Description is or becomes blank), so they can reuse a prior entry (name + amount) with one tap. This serves the "fast logging" signature element (UX brief) and Guiding Principle #2 (minimal user input) for the app's habitual, recurring-expense use case, and also helps refine a blank or mistyped Description when editing.
- **Acceptance Criteria:**
  - **Reuse name + amount, with amount provenance.** Tapping a suggestion populates the name and — when the draft amount is empty or was seeded by a previous tile tap — the amount. A **user-typed amount survives the tap** (fresher intent beats the tile's historical amount; a silent overwrite the user misses before Save is the costly failure). A **double-tap** (or the "Replace amount and description" VoiceOver custom action) performs a full replace of both fields, overriding provenance. (Amount is the required field and the primary friction-saver; see F-2.04.) In Edit mode, the seeded existing amount is treated as user-typed for this rule: a single tap fills Description only; the double-tap (or clearing the amount first) is the explicit path to also take the tile's amount.
  - **Fill, don't auto-save.** A tap populates the draft fields and leaves the user on the sheet to review, edit, and Save. It does **not** log or commit instantly. The amount remains visible and editable so a stale or wrong value is caught before Save (Guiding Principle #3, Clarity).
  - **Suggestion → autocomplete on the same surface.** The Recents section renders **directly below the Description field** (input-above/suggestions-below autocomplete idiom; the Amount card keeps the top slot for amount-first quick logging). As the user types into the Description field, the suggestion set narrows to matching recents — the typed query searches the full memoized corpus, not just the visible row. There is no separate picker mode or screen; the surface stays anchored on the form rather than pulling the user off course. Clearing the Description (including via its trailing clear button) restores the unfiltered row.
  ...
  - **Edit-mode reveal (blank-Description-gated, persist-once-shown).** In Edit/View mode the Recents section is hidden by default and revealed once the Description draft has been blank (trimmed empty) — either because the expense opened with a nil/whitespace name, or because the user cleared it mid-session. Once revealed, the section persists for the rest of the sheet's lifetime (the latch is one-way: it does not hide again when the Description becomes non-empty). All Add-mode affordances apply identically once revealed: filter-as-you-type, same tile row, same tap/double-tap semantics.
- **Edge Cases / Notes:**
  - **Ranking.** Recency-driven. Each unique description (case/diacritic-folded) gets a **base tile** carrying its most recent amount; additional **amount-variant tiles** surface only when that exact (name, amount) pair recurred ≥ 3 times (max 2 extras per name, clustered after their base tile), so a two-price regular gets both as one-tap tiles while one-off price jitter stays collapsed. The memoized **search corpus** (cap 200) is decoupled from the **display cap** (30) so typed queries reach beyond the visible row. Constants pinned by unit test; subject to revision once usage data accrues.
...
- **Description:** Allow each Budget to have its own period start date via a new `Budget.startDate` field. The start date may be in the past (back-filling), today, or in the future. `startDate` defines when the budget's window begins; for biweekly Budgets it is additionally the cycle anchor (`biweeklyAnchor` = `startDate`). Weekly Budgets grid on the global `AppSettings.weekStartDay` (see F-5.01, change `weekly-global-week-start`) — a weekly `startDate` off that grid clips the first period without proration, like a monthly Budget created mid-month. The Add/Edit Budget screen pre-populates `startDate` per period type and requires a value before Save.
- **Acceptance Criteria:**
