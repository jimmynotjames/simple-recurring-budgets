...
- **Acceptance Criteria:**
  ...
  - **For recurring period types** (`daily`, `weekly`, `biweekly`, `monthly`):
    ...
    - **Carry-over** field — separate signed cumulative carryover per [main-prd.md §6.7](main-prd.md#67-carry-over-behavior). The chip updates **mid-period** in response to any change that affects the value (expense add / edit / delete, allocation edit, start/end-date edit, pause/resume, manual reset) — not only at period boundaries. Refresh is driven by user actions and app-lifecycle events (app launch, `scenePhase == .active`); there is intentionally no timer-driven refresh for clock-driven period-boundary crossings.
...
- **Acceptance Criteria:**
  ...
  - **Mid-period refresh** — the chip values update mid-period in response to any change that affects them (expense add / edit / delete, allocation edit, start/end-date edit, pause/resume, manual reset). Refresh is driven by user actions and app-lifecycle events (app launch, `scenePhase == .active`); there is no timer-driven refresh for clock-driven period-boundary crossings.
  ...
  - **Primary Add Expense action** — a full-width `.borderedProminent` button presenting `SheetRoute.addExpense(budget)` for this specific budget. The CTA is **hidden or disabled** while the budget is paused, before `startDate`, or after `endDate`. (Backdated edits to existing expenses in prior active periods remain available — see F-2.04 date-bounds rule.)
...
- **Acceptance Criteria:**
  ...
  - **Date bounds** — the date picker constrains the user's selection to `[Budget.startDate, Budget.endDate]` ∩ the union of the budget's active periods (where "active period" is defined by the budget's pause/resume history — see F-7.06). Out-of-range dates are either not offered by the picker or rejected on Save. For Specific Dates budgets (F-2.08), this collapses to `[startDate, endDate]`. The picker default in Add mode is now (clamped into the allowed range when needed).
...
- **Acceptance Criteria:**
  ...
  - **Allocation edits while paused** take effect at the resume point — they apply to the period containing the Resume action and every subsequent active period (see F-2.03).
  - **Backdated expenses to prior active periods** are always accepted (whether the budget is currently paused or active) and recompute the carry-over of the period they fall in; that change propagates forward through all subsequent active periods. The Add/Edit Expense date picker is constrained to the union of active periods while paused (see F-2.04).
