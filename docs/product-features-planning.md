...
- **Status:** Implemented. `AppSettings.weekStartDay` with Settings picker and confirmation alert; implemented by change `settings-screen`. Global-grid scope (below) shipped with `weekly-global-week-start` (issue #240), reversing the per-budget-anchor scope clarification that had shipped with `rewrite-budget-calculations`.
