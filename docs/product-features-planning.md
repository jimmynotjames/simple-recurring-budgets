...
- **Status:** Implemented. Implemented by change `finish-recently-used-expenses` (workshop design landed pre-change; cross-cutting concerns and tests landed with the change). Reworked by change `recents-rework-spec-sync` (placement below Description, recurrence-gated amount variants, corpus/display cap split, amount-provenance smart apply, double-tap full replace, Description clear button).
