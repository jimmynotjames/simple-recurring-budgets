...
The app SHALL expose exactly three `os.Logger` constants in `simple-recurring-budgets/Logging/AppLoggers.swift`, each scoped to the app's bundle subsystem (or the fallback `"simple-recurring-budgets"` when `Bundle.main.bundleIdentifier` is nil):
...
These three categories are the canonical set. Adding a new category to `AppLoggers.swift` SHALL be accompanied by a new requirement in this spec in the same change. Removing or renaming a category SHALL be accompanied by a `MODIFIED Requirement` block in the same change.

#### Scenario: All three logger constants are reachable from app code

- **WHEN** application code references `Logger.bootstrap`, `Logger.cloudKit`, or `Logger.ui`
- **THEN** the constants resolve to `os.Logger` values whose `subsystem` equals `Bundle.main.bundleIdentifier ?? "simple-recurring-budgets"` and whose `category` equals `"bootstrap"`, `"cloudkit"`, or `"ui"` respectively

#### Scenario: AppLoggers exposes no fourth category

- **WHEN** a contributor inspects `AppLoggers.swift`
- **THEN** exactly three `Logger` constants are declared, named `bootstrap`, `cloudKit`, and `ui`
...
- `Logger.cloudKit.error("cloudkit.container.failed: \(error.localizedDescription, privacy: .public)")` — both creation paths failed; about to crash via `fatalError`.
...
- **THEN** `Logger.cloudKit.error` is emitted with the localized error description interpolated at `privacy: .public`, before `fatalError(_:)` terminates the process
...
[`docs/tech-design-doc.md`](../../../docs/tech-design-doc.md) §7 SHALL list the same three category strings (`bootstrap`, `cloudkit`, `ui`) as `AppLoggers.swift`. Any change to the category set in `AppLoggers.swift` SHALL update §7 in the same OpenSpec change. Cross-reference to [`docs/analytics-spec.md`](../../../docs/analytics-spec.md) §17 (the OSLog ↔ `AnalyticsClient` boundary) SHALL be present in §7 so a reader of either doc can find the other.

#### Scenario: Tech-design-doc §7 matches AppLoggers.swift

- **WHEN** a reader inspects `docs/tech-design-doc.md` §7
- **THEN** the three categories listed there are exactly `bootstrap`, `cloudkit`, and `ui`, matching `AppLoggers.swift`
