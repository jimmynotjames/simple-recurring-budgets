...
SwiftUI sheet used for both creating a new `Budget` and editing an existing one. Provides a card-based form (Name, Allocation/Currency, Period, Carry-Over) backed by `AddEditBudgetViewModel`, wired into the centralised `Router.sheet` mechanism. Satisfies F-2.03 and the currency-picker path of F-3.04.
## Requirements
...
- **THEN** the form shows: name field empty (showing `"Budget"` placeholder), allocation field empty (draft `nil`), currency code from the user's locale (or `"USD"` if locale lookup returns `nil`), period `Daily` selected, Carry-Over toggle ON; Save is disabled because the name is empty and allocation is unset
...
When any condition fails, the Save button SHALL be disabled. Disabled state SHALL use the system disabled appearance; no inline error text is displayed in either MockupB or this implementation.
...
- **WHEN** the trimmed name is non-empty AND `allocation != nil` AND the allocation is `> 0`
...
1. Build a `Budget` using `Budget.init(name:allocation:currencyCode:period:resetCadence:isCarryOverEnabled:)` with: the drafted `name` (post-trim, but the model stores the user's value as entered; trimming is for validation only), the drafted **unwrapped** `allocation` (non-`nil` positive `Decimal` at this point), the drafted `currencyCode`, the drafted `period`, `resetCadence: nil` (Reset Cadence is paused; `Budget.init` defaults to `.never`), and the drafted `isCarryOverEnabled`.
