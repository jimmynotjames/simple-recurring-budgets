...
The expanded title SHALL be the first row of the screen's `List`, leading-aligned to the outer edge of the inset-grouped cards below it, with the budget's `icon` (when set) rendered immediately before the budget `name` as a single leading title (decorative icon). It SHALL use large-title font size with bold weight, support Dynamic Type, and wrap to multiple lines (no single-line truncation) so a long name displays in full; wrapped lines SHALL align to the leading margin (the icon offsets only the first line, not subsequent lines). The icon SHALL sit on the leading edge and mirror with layout direction (right edge in RTL). When no icon is set, only the name is rendered.

As the user scrolls the list up, once the expanded title has substantially scrolled beneath the navigation bar, an inline navigation-bar title SHALL fade in. The inline title SHALL render the same icon (decorative) + name in the navigation bar, on the leading edge, truncated to a single line. When the user scrolls back to the top, the inline title SHALL fade back out and the expanded content title SHALL be fully visible.
...
- **THEN** the expanded content title renders the icon immediately before the name on the leading edge — on the left in LTR and on the right in RTL — rather than pinning the emoji to the left
...
- **THEN** the expanded content title wraps to multiple lines and displays the full name, with wrapped lines aligned to the leading margin (not indented under the icon) and the title does not truncate
...
- **THEN** an inline navigation-bar title (icon on the leading edge + single-line truncated name) fades in; scrolling back to the top fades it out again

#### Scenario: Title without an icon

- **WHEN** the detail screen is shown for a budget with no icon
- **THEN** both the expanded content title and the inline nav-bar title render the budget name alone, with no leading icon
...
- **VoiceOver label (header).** The header's composed VoiceOver label is produced by the same `BudgetRemainingSummary.accessibilityLabel(...)` helper used by the Budgets-list row, with `budgetName: nil` since the nav title already announces the budget name. The helper dispatches to two dedicated keys for the preStart and postEnd cases (the paused case continues to use the existing on-budget / over-budget keys):
