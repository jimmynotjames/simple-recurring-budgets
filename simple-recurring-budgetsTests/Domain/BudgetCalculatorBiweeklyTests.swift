...
struct BudgetCalculatorBiweeklySnapshotTests {
  @Test func multiCycleCarryOverWalk_sumsCompletedCycles() {
    ...
    let snap = BudgetCalculator.snapshot(budget: budget, expenses: expenses, now: d(2026, 5, 1), calendar: cal)
  ...
  @Test func allocationChangeAtCycleBoundary_appliesPerCycle() {
    ...
    let snap = BudgetCalculator.snapshot(budget: budget, expenses: [], now: d(2026, 4, 29), calendar: cal)
  ...
  @Test func pauseResumeAcrossCycles_pausedCycleContributesZero() {
    ...
    let snap = BudgetCalculator.snapshot(budget: budget, expenses: [], now: d(2026, 5, 30), calendar: cal)
  ...
  @Test func endDateBeforeFirstCycleCompletes_singleTruncatedWindow() {
    ...
    let snap = BudgetCalculator.snapshot(budget: budget, expenses: [exp], now: d(2026, 4, 25), calendar: cal)
